...
// DB first => Right click entities directory > New > JPA Entities from DB
...
// Use one technique, if writing models is easier to describe entities, do so then
// apply the flyway migration then clean-up the directives.
@Setter
@Getter
@AllArgsConstructor
@NoArgsConstructor
...
public class User {
    ...
    // we use cascade type persist to save(ONLY, for delete use .REMOVE) related entities in one transaction ( Instead of writing two statements)
